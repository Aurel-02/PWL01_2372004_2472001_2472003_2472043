<x-app-layout>
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
                <h2 class="font-black text-4xl text-white leading-tight tracking-tight uppercase">
                    TERMINAL <span class="text-secondary">SCAN TIKET</span>
                </h2>
                <p class="text-pearl font-bold uppercase tracking-widest text-[10px] mt-2">Gunakan terminal ini untuk verifikasi tiket masuk tamu.</p>
            </div>
            <div class="flex items-center gap-2 bg-secondary/20 backdrop-blur-md px-6 py-3 rounded-2xl border border-secondary/30 shadow-[0_0_30px_rgba(16,185,129,0.2)]">
                ...
                <span class="text-secondary-400 text-xs font-black uppercase tracking-widest">Status: Aman</span>
            </div>
        </div>
    </x-slot>
    ...
    <div class="max-w-3xl mx-auto py-12 px-4 sm:px-6 lg:px-8">
        <div class="bg-white rounded-[3rem] shadow-2xl shadow-secondary/20 overflow-hidden border border-white">
            <div class="p-12">
                <div class="text-center mb-12">
                    ...
                    <h3 class="text-3xl font-black text-primary uppercase tracking-tight mb-2">VALIDASI AKSES</h3>
                    <p class="text-gray-500 font-bold text-xs uppercase tracking-widest">Masukkan kode 8-digit tiket di bawah ini.</p>
                </div>
                ...
                @if(session('success'))
                    <div class="mb-8 bg-secondary-50 border-2 border-secondary-100 p-8 rounded-[2rem] flex flex-col items-center text-center animate-bounce-short">
                        ...
                        <h4 class="text-secondary-900 font-black uppercase tracking-tight text-xl mb-1">Verifikasi Berhasil</h4>
                ...
                @if(session('error'))
                    <div class="mb-8 bg-rose-50 border-2 border-rose-100 p-8 rounded-[2rem] flex flex-col items-center text-center">
                        ...
                        <h4 class="text-rose-900 font-black uppercase tracking-tight text-xl mb-1">Akses Ditolak</h4>
                        <p class="text-rose-600 font-bold text-sm">{{ session('error') }}</p>
                    </div>
                @endif

                <form action="{{ Auth::user()->isAdmin() ? route('admin.scan.process') : route('organizer.scan.process') }}" method="POST" class="space-y-6">
                    @csrf
                    <div>
                        <input type="text" name="ticket_code" 
                            class="w-full bg-gray-50 border-2 border-gray-100 focus:border-secondary focus:ring-0 rounded-3xl px-8 py-6 text-center text-3xl font-black tracking-[0.5em] text-primary placeholder-gray-200 transition-all"
                    ...
                    <button type="submit" class="w-full bg-secondary hover:bg-black text-white font-black uppercase tracking-[0.2em] py-6 rounded-3xl shadow-xl shadow-secondary/20 transition-all transform hover:-translate-y-1 active:scale-95 text-sm">
                        VERIFIKASI AKSES &rarr;
                    </button>
                </form>

                <div class="mt-12 pt-12 border-t border-gray-50 grid grid-cols-2 gap-8">
                    <div class="text-center p-6 bg-gray-50 rounded-[2rem] border border-white">
                        <p class="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1">Total Scan</p>
                        <p class="text-3xl font-black text-primary">{{ $totalScans ?? 0 }}</p>
                    </div>
                    <div class="text-center p-6 bg-gray-50 rounded-[2rem] border border-white">
                        <p class="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1">Hari Ini</p>
                        <p class="text-3xl font-black text-secondary">{{ $todayScans ?? 0 }}</p>
                    </div>
                </div>
            </div>
        </div>
        ...
        <p class="mt-8 text-center text-[10px] font-black text-gray-400 uppercase tracking-[0.3em]">
            Siap untuk Check-in tamu di lokasi.
        </p>
    </div>
</x-app-layout>
